...
void TestTextureIO::read_data()
{
    ...
    QTest::newRow("64x64, RGBA_8888, 1, 1") << 64 << 64 << 1 << TextureFormat::RGBA8_Unorm << 1 << 1;
}
...
void TestTextureIO::read()
{
    ...
    auto expectedTexture = Texture(format, {width, height, depth}, {layers, levels});
...
void TestTextureIO::write()
{
    ...
    auto expectedTexture = Texture(format, {width, height, depth}, {levels, layers});
