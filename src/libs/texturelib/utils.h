...
std::pair<GLenum, GLuint> bindTexture(OpenGLFunctions *funcs, const Texture &texture)
{
    ...
    case Texture::Type::Texture1D:
        ...
        for (int level = 0; level < texture.levels(); ++level) {
            funcs->glTexImage1D(GL_TEXTURE_1D, level, GL_RGB, texture.width(level), 0, GL_BGR, GL_UNSIGNED_BYTE, texture.data(level));
        }
    ...
    case Texture::Type::Texture1DArray:
        ...
        for (int level = 0; level < texture.levels(); ++level) {
            funcs->glTexImage2D(GL_TEXTURE_1D_ARRAY, level, GL_RGB, texture.width(level), texture.layers(), 0, GL_BGR, GL_UNSIGNED_BYTE, texture.data(level));
        }
    ...
    case Texture::Type::Texture2D:
        ...
        for (int level = 0; level < texture.levels(); ++level) {
            funcs->glTexImage2D(GL_TEXTURE_2D, level, GL_RGB, texture.width(level), texture.height(level), 0, GL_BGR, GL_UNSIGNED_BYTE, texture.data(level));
        }
        if (texture.levels() == 1)
    ...
    case Texture::Type::Texture2DArray:
        ...
        for (int level = 0; level < texture.levels(); ++level) {
            funcs->glTexImage3D(GL_TEXTURE_2D_ARRAY, level, GL_RGB, texture.width(level), texture.height(level), texture.layers(), 0, GL_BGR, GL_UNSIGNED_BYTE, texture.data(level));
        }
    ...
    case Texture::Type::Texture3D:
        ...
        for (int level = 0; level < texture.levels(); ++level) {
            funcs->glTexImage3D(GL_TEXTURE_3D, level, GL_RGB, texture.width(level), texture.height(level), texture.depth(level), 0, GL_BGR, GL_UNSIGNED_BYTE, texture.data(level));
        }
